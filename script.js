var themes = {
    ...
    frutas: ["ABACAXI", "UVA", "MORANGO", "MANGA", "GOIABA"],
...
function start() {
    ...
    initializeGame();
    //startTimer(); Desativado temporariamente
}
...
function initializeGame() {
    ...
    if (remainingWords.length === 0) {
        // Tema completo, marcar como concluído
        completedThemes[selectedTheme] = true;
        updateThemeStyles();  // Atualiza o estilo da lista de temas
        alert("Parabéns! Você completou todas as palavras!");
        clearInterval(timerInterval);
        returnToStartScreen(); // Voltar a tela inicial
        return;
    }
...
function updateThemeStyles() {
    ...
    // Atualiza o estilo de cada tema baseado em `completedThemes`
    for (let i = 0; i < options.length; i++) {
        const themeOption = options[i];
        if (completedThemes[themeOption.value]) {
            themeOption.style.backgroundColor = '#4CAF50';  // Tema completado (verde)
            themeOption.style.color = '#FFF';
        } else {
            themeOption.style.backgroundColor = '#333';  // Tema não completado (cinza)
            themeOption.style.color = '#DDD';
        }
    }
}
...
function checkWord() {
    ...
    if (correct.join('') === word) {
        ...
        // Adiciona classes para animação de erro
        sentenceBox.querySelectorAll('.dropzone').forEach(element => {
            element.classList.add('wrong');
        });

        setTimeout(() => {
            // Remove classes de erro após 500ms
            sentenceBox.querySelectorAll('.dropzone').forEach(element => {
                element.classList.remove('wrong');
            });

            // Limpa e restaura o jogo como antes
...
function returnToStartScreen() {
    // Esconder a área do jogo
    document.querySelector(".container").style.display = "none";
    // Mostrar a tela inicial de escolha de tema
